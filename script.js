...
function enterFullscreen() {
    ...
    if (request) Promise.resolve(request.call(element)).catch(() => {});
}

document.addEventListener("touchstart", enterFullscreen, { passive: true });

function updateHUD() {
    score.textContent = "Score : " + game.score + " | Level : " + game.level;
}
...
function showLevelUp() {
    ...
    setTimeout(() => levelUp.classList.remove("show"), 1000);
}
...
function updateLevel() {
    const newLevel = Math.min(MAX_LEVEL, game.startLevel + Math.floor(game.score / 10));
...
function resetGame() {
    ...
    game.lastSpawn = 0;
    if (game.animationId !== null) cancelAnimationFrame(game.animationId);
...
function createCloud() {
    ...
    const cloud = { x: -180, y: 40 + Math.random() * 220, speed: 0.4 + Math.random() * 1.2, width, element };
...
function hitPlayer(bomb) {
    ...
    return bomb.x + bomb.size - 8 > rect.left + 30 &&
...
function spawnClouds() {
    if (game.clouds.length < 5 && Math.random() < 0.03) createCloud();
}
...
document.addEventListener("keydown", event => {
    if (!game.running) return;
    if (["ArrowLeft", "a", "A"].includes(event.key)) game.moveLeft = true;
    if (["ArrowRight", "d", "D"].includes(event.key)) game.moveRight = true;
});

document.addEventListener("keyup", event => {
    if (["ArrowLeft", "a", "A"].includes(event.key)) game.moveLeft = false;
    if (["ArrowRight", "d", "D"].includes(event.key)) game.moveRight = false;
});
