...
function setRegisterMode(enabled) {
    ...
    document.getElementById("memberFormTitle").textContent = enabled ? "Buat Akun Member" : "Login Member";
    loginMember.textContent = enabled ? "DAFTAR MEMBER" : "LOGIN MEMBER";
    registerMember.textContent = enabled ? "KEMBALI KE LOGIN" : "BUAT AKUN MEMBER";
    memberPassword.placeholder = enabled ? "Password minimal 6 karakter" : "Password";
}
...
async function loginOrRegister() {
    ...
    if (!/^[a-zA-Z0-9_]{3,20}$/.test(username)) {
        showMemberMessage("Username harus 3-20 karakter: huruf, angka, atau underscore.", true);
        return;
    }

    if (password.length < 6) {
        showMemberMessage("Password minimal terdiri dari 6 karakter.", true);
        return;
    }
    ...
    const endpoint = registerMode ? "/api/member/register" : "/api/member/login";
...
async function restoreMemberSession() {
    ...
    try {
        ...
        if (!response.ok || !result.loggedIn) throw new Error("invalid");
...
function startMemberHeartbeat() {
    ...
    memberHeartbeat = setInterval(async () => {
        ...
        try {
            ...
            if (!response.ok) await logoutMemberAccount(true);
        } catch {}
    }, 10000);
}
...
window.addEventListener("beforeunload", () => {
    if (!memberToken) return;
    navigator.sendBeacon("/api/member/logout", new Blob([], { type: "application/json" }));
});
